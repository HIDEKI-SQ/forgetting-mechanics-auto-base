# -*- coding: utf-8 -*-
"""
C4 Implementation (Minimal Core; ref/auto/delta)
...
# === auto renderer（H1だけC1、H2/H3はパラフレーズ） ===
...
def main():
    ROOT = pathlib.Path(".")
    OUT = ROOT / "outputs" / "c4_impl"

    case = os.environ.get("TARGET_ID","brand_larica")
    bp_path = os.environ.get("BP_PATH","") or f"outputs/c2_manual/bp_{case}.yaml"
    out_dir = OUT / case
    out_dir.mkdir(parents=True, exist_ok=True)

    bp = yaml.safe_load(pathlib.Path(bp_path).read_text(encoding="utf-8"))

    # ref（人手）。存在しない場合は delta=None のまま
    ref_basic = read_text(pathlib.Path(f"inputs/c4/{case}/ref_basic.md"))
    ref_brief  = read_text(pathlib.Path(f"inputs/c4/{case}/ref_briefing.md"))

    # auto（決定論テンプレ）
    auto_basic = render_auto_paragraphs(bp, "basic")
    auto_brief = render_auto_paragraphs(bp, "briefing")

    # 保存
    write_text(out_dir/"regen_auto_basic.md", auto_basic)
    write_text(out_dir/"regen_auto_briefing.md", auto_brief)
    if ref_basic: write_text(out_dir/"regen_ref_basic.md", ref_basic)
    if ref_brief: write_text(out_dir/"regen_ref_briefing.md", ref_brief)

    # 測定
    sp_auto = round((sp_l2(bp, auto_basic)+sp_l2(bp, auto_brief))/2, 2)
    vs_auto = vs_proxy(auto_basic, auto_brief)

    SP_ref = round((sp_l2(bp, ref_basic)+sp_l2(bp, ref_brief))/2, 2) if (ref_basic and ref_brief) else None
    VS_ref = vs_proxy(ref_basic, ref_brief) if (ref_basic and ref_brief) else None

    metrics = {
        "meta":  {"mode":"c4_impl","case_id":case,"bp_path":bp_path},
